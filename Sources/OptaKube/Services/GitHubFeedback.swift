...
enum FeedbackKind: String, CaseIterable, Identifiable {
    ...
    var placeholder: String {
        switch self {
        case .bug:
            return "What happened, what you expected, and the steps to reproduce it. Cluster type (EKS, kind, …) and the resource involved help."
        case .feature:
            return "What you'd like OptaKube to do, and why it'd help your workflow."
        case .question:
...
enum GitHubFeedback {
    static let repo = "souriscloud/optakube"

    /// Non-identifying environment line appended to the report. App version/build,
    /// macOS version, and CPU arch only — never kubeconfig, cluster, or token data.
    static func diagnostics() -> String {
        ...
        let arch = "x86_64"
        #endif
        return "OptaKube \(AppInfo.version) (build \(AppInfo.build)) · macOS \(osStr) · \(arch)"
    }

    /// Full report text (body + optional diagnostics) — used for both the GitHub
    /// URL and the "Copy report" fallback.
